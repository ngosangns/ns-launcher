...
`scripts/bundle-app.sh` — they must not spell those steps out separately. The
script copies SwiftPM's `ns-launcher_NSLauncherApp.bundle` when `Package.swift`
lists `resources:`. The launcher currently ships none.
